...
import { getTransactions } from "@/app/actions/get";

/**
 * GET /api/user/transactions
 *
 * Fetches payment transactions for the authenticated user.
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Fetch transactions filtered by authenticated user's ID
    const { transactions, pagination, stats } = await getTransactions({
      userIds: [user.id], // Filter by authenticated user only
      search: search || null,
      sortBy: validSortBy,
    ...
    console.error("user transactions route error:", err);
    const errorMessage =
