...
/**
 * Integration tests for Admin Rewards module
 *
 * Tests:
...
describe("Admin Rewards Module", () => {
  ...
  it("should load rewards page and capture screenshot", async () => {
    const result = await testAdminPage(page, "/admin/rewards", "rewards");

    expect(result.screenshotPath).toBeDefined();
    expect(result.screenshotPath).toContain("admin-rewards");

    // Verify screenshot file exists
    expect(fs.existsSync(result.screenshotPath)).toBe(true);
  }, 60000);

  it("should pass Lighthouse audit for rewards page", async () => {
    const result = await testAdminPage(page, "/admin/rewards", "rewards");
