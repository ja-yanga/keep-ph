...
/**
 * Integration tests for Admin Packages module
 *
 * Tests:
 ...
 */
describe("Admin Packages Module", () => {
  let browser: Browser;
  let page: Page;
  ...
  it("should load packages page and capture screenshot", async () => {
    const result = await testAdminPage(page, "/admin/packages", "packages");

    expect(result.screenshotPath).toBeDefined();
    expect(result.screenshotPath).toContain("admin-packages");

    // Verify screenshot file exists
    expect(fs.existsSync(result.screenshotPath)).toBe(true);
  }, 60000);

  it("should pass Lighthouse audit for packages page", async () => {
    const result = await testAdminPage(page, "/admin/packages", "packages");
