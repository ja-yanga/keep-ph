...
/**
 * Integration tests for Admin Stats module
 *
 * Tests:
...
describe("Admin Stats Module", () => {
  ...
  it("should load stats page and capture screenshot", async () => {
    const result = await testAdminPage(page, "/admin/stats", "stats");

    expect(result.screenshotPath).toBeDefined();
    expect(result.screenshotPath).toContain("admin-stats");

    // Verify screenshot file exists
    expect(fs.existsSync(result.screenshotPath)).toBe(true);
  }, 60000);

  it("should pass Lighthouse audit for stats page", async () => {
    const result = await testAdminPage(page, "/admin/stats", "stats");
